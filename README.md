...
> [!IMPORTANT]
> **带内核级反作弊的游戏属于风险区。** 反作弊可能拦截并隔离代理 DLL，检测记录还可能危及账号——程序会检测并**提示风险，是否部署由你决定**，详见[反作弊章节](#反作弊风险评估由你决断)。
...
- [关于显卡](#关于显卡)
- [更新 mod 文件](#更新-mod-文件)
- [添加代理 DLL（社区构建的入口）](#添加代理-dll社区构建的入口)
- [目录结构](#目录结构)
- [主题与多语言](#主题与多语言)
- [从源码构建](#从源码构建)
- [授权](#授权)
...
上方工具条显示当前 Mod 文件源和你的显卡；右侧是扫描、下载与「添加代理 DLL…」按钮（后者见[添加代理 DLL](#添加代理-dll社区构建的入口)）。窗口右上角有四个控件：
...
本 mod 是 DLL 代理，而内核级反作弊专门盯这类东西，所以这类游戏属于**风险区**：反作弊可能在游戏启动前就把代理 DLL 拦截并改名隔离（例如留下 `version.dll.3787982156`），也可能记录检测信息、危及账号。

实测确认的案例：**绝区零**内置米哈游 HoYoKProtect，检测到 `version.dll` 后直接改名隔离，游戏随即弹出 `The client component is running abnormally, please restart the client. Error Code:(0,11008,2195210578)`。但同一个游戏上，社区构建的 `d3d12.dll` 入口可以正常工作——见[添加代理 DLL](#添加代理-dll社区构建的入口)。

程序会扫描游戏目录**及其上溯 3 层**，识别这些反作弊并在部署前提示风险：
...
检测到内核级反作弊时，程序**只提示、不禁止**：

- **添加游戏时立即弹窗**说明风险（若一次扫描出多个，合并成一条汇总提示，不逐条弹）；
- 配置面板顶部显示橙色警告条；
- 点「部署到该游戏」会弹出确认框，**默认选"否"**；确认后才写入；
- 「全部部署」的确认框把这类游戏单独列出来，确认后一并部署；
- 「一键恢复」始终可用，用于清理残留。

> **为什么不禁用**：扫描只能读到反作弊**在不在**，读不到它**会不会拦你选的那个入口名**。绝区零上 `version.dll` 会被隔离，而 `d3d12.dll` 能正常工作——这个判断只有你自己能做，所以程序负责把证据和后果摆清楚。

**实测记录**（开发机上扫描到的游戏，供参考）：

| 游戏 | 反作弊 | 结论 |
|---|---|---|
| 绝区零 | 米哈游 HoYoKProtect | ⚠ 自带入口 `version.dll` 被隔离；社区入口 `d3d12.dll` 可用 |
| 战争雷霆 | BattlEye | 未测试 |
| 终末地 | 腾讯 ACE | 未测试 |
| 守望先锋 | 网易 NEAC | 未测试 |
...
**在反作弊游戏上部署之后**：如果游戏报错、或帧生成没有生效，说明这个入口名被拦了——换一个入口名重试，或直接「一键恢复」。检测记录带来的账号风险由你自己承担。
...
> 除了上游的 mod 文件，这一步还会获取本仓库分发的**附加入口** `d3d12.dll`（约 10 MB，按固定哈希校验，已存在则跳过），详见[添加代理 DLL](#添加代理-dll社区构建的入口)。
...
本项目只发布 5 个入口名。有些游戏的保护模块会抢先占用这几个名字——**绝区零**就是例子——社区因此会编译其他入口，最常见的是 `d3d12.dll`：游戏在初始化 DX12 后端时才动态加载它，那时代理有机会被加载。

**`d3d12.dll` 已随本仓库分发，不需要你手动准备**：点「下载 / 更新 Mod 文件」（安装向导勾选「Mod 文件」时也会做同样的事）就会自动获取，落到 `mod\altnative\d3d12.dll`，然后在每款游戏的「代理入口」下拉里出现，标注"附加入口"。

这个文件管理器无法校验签名（不是本项目构建的），所以改用**固定 SHA-256**：字节不符即丢弃，换哪个下载源都一样。哈希记录在源码里（`ModFetcher.Extras`），文件与来源说明见 [extra-proxies/README.md](extra-proxies/README.md)。

如果要用**别的**入口 DLL（自己编译的、或社区的其他构建），点工具条上的「**添加代理 DLL…**」选中文件即可。程序会：

- 按**原文件名**把它复制到 `mod\altnative\` —— 文件名就是入口名，游戏按这个名字解析，所以名字不能改；
- 只做只读检查并把**签名者**写进日志（有签名则显示证书主体，没有则明确标注），不校验来源：文件是你提供的，来源由你确认；
- 之后它就和自带入口一样出现在「代理入口」下拉里（标注"附加入口"），可部署、可一键恢复。恢复时按部署记录里的 SHA256 删除，不会碰别的文件。

两条限制：不允许覆盖自带的 5 个入口名（那会顶掉签名校验所依赖的官方构建）；文件名不在已知入口列表里（`version` / `winmm` / `dinput8` / `winhttp` / `dxgi` / `d3d12`）时会提示"游戏很可能不会加载它"，是否继续由你决定。下载器发现 `mod\altnative\d3d12.dll` 已存在且与仓库版本不同时，**会保留你的文件**，所以放自己的构建进去不会被更新覆盖。

> **为什么不能随便起名字**：入口名是"游戏会去加载的 DLL 名"。名字对了，代理才会被载入游戏进程；名字错了，部署完什么也不会发生。这也是管理器**不会**把 `version.dll` 改名成 `d3d12.dll` 部署的原因——每个构建只导出它对应名字所需的系统 API，改名会让游戏的 D3D12 导入找不到实现，游戏直接起不来。
