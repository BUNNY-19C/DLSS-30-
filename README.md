...
点工具条上的「**添加代理 DLL…**」选中文件即可。程序会：

- 按**原文件名**把它复制到 `mod\altnative\` —— 文件名就是入口名，游戏按这个名字解析，所以名字不能改；
- 只做只读检查并把**签名者**写进日志（有签名则显示证书主体，没有则明确标注），不校验来源：文件是你提供的，来源由你确认；
- 之后它就和自带入口一样出现在每款游戏的「代理入口」下拉里（标注为"本地导入"），可部署、可一键恢复。恢复时按部署记录里的 SHA256 删除，不会碰别的文件。

两条限制：不允许覆盖自带的 5 个入口名（那会顶掉签名校验所依赖的官方构建）；文件名不在已知入口列表里（`version` / `winmm` / `dinput8` / `winhttp` / `dxgi` / `d3d12`）时会提示"游戏很可能不会加载它"，是否继续由你决定。
...
测试（358 项，覆盖部署/恢复/备份保护/反作弊识别与拦截/入口名管理/本地代理导入/目录解析/接管/INI 渲染/持久化/下载 URL 策略/签名校验/下载源选择/主题与多语言）：
