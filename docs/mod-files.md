...
另外，管理器会从**本仓库**（`extra-proxies/d3d12.dll`）下载社区构建的 `d3d12.dll` 放进这个目录（界面上的下载按钮与安装器的「Mod 文件」任务都会做）。它是按**固定 SHA-256** 校验的，已存在且与仓库版本不同时会被保留——所以放自己的构建进去不会被覆盖。详见 [../extra-proxies/README.md](../extra-proxies/README.md) 与 README 的「添加代理 DLL（社区构建的入口）」。
